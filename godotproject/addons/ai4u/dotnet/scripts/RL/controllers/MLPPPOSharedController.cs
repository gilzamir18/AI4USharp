...
namespace ai4u
{
    ...
    /// </summary>
    public partial class MLPPPOSharedController: Controller
	{
		...
		public override void OnSetup()
		{
            ...
            for (int i = 0; i < GetChildCount(); i++)
            {
                ...
                {
                    controller = (MLPPPOController) child;
                    ((MLPPPOController) controller).model = model;
                    controller.agent = agent;
                    controller.OnSetup();
                    break;
                }
                else if (child is TrainController && model.IsInTrainingMode)
                {
                    controller = (TrainController) child;
                    controller.agent = agent;
                    controller.OnSetup();
                    break;
                }
